...
data['A_routes'] = [('P1', 'D1'), ('D1', 'R1')]

# Predecessors to facility i
n_i_init = {}
n_i_init['P1'] = ['D1']
n_i_init['D1'] = ['R' + str(i) for i in range(1, 2)]
data['succ'] = n_i_init
...
data['ship_lead_time'] = {('P1', 'D1'): 1, ('P2', 'D1'): 1, ('D1', 'R1'): 1, ('D1', 'R2'): 1, ('D1', 'R3'): 1,
                          ('D1', 'R4'): 1}
...
# Production costs
data['production_costs'] = {'P1': 5, 'P2': 3}
# Backorder penalty
data['backorder_penalty'] = {i:1000 for i in data['R_set']}
# Waste costs
data['waste_penalty'] = {i:1500 for i in data['R_set'] + data['D_set'] + data['P_set']}
...
data['shipment_UB'] = {(i, j): [50000 for _ in range(data['H'])] for i, j in data['A_routes']}
# Production upper bound (production capacity)
data['production_UB'] = {i: [40000 for _ in range(data['H'])] for i in data['P_set']}
